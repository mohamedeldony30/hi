...
const jsonURL = 'https://raw.githubusercontent.com/555-Li/module/main/imagen_json/anime.json'
...
const reply = await conn.reply(m.chat, '> *¡Personaje agregado exitosamente!*\n\nResponde a este mensaje con "enviar" o "👍" sólo si deseas enviar los personajes a mis creadores para que lo agreguen en *zenon*.', m)
...
await conn.reply('201061419002@s.whatsapp.net', `*Solicitud de @${m.sender.split("@")[0]} Para agregar personajes de Fantasy RPG en zenon*`, null, { mentions: [m.sender] })
await conn.sendMessage('201061419002@s.whatsapp.net', { document: databaseFantasyAdd, mimetype: 'application/json', fileName: `fantasyAdd_${m.sender}.json` }, { quoted: m })
await conn.reply('201061419002@s.whatsapp.net', `${jsonString}`, m)
await conn.reply(m.chat, `¡Archivo enviado a mis creadores! Sigue agregando más personajes que quieras que esten en zenon`, m)
}}  
} catch (error) {
